...
if(isset($_POST['update_product'])){
   ...
   $row = mysqli_fetch_assoc($select_product_name);
   
   if(mysqli_num_rows($select_product_name) > 0 && $update_p_id != $row['id']){
      $message[] = 'product name already exist!';
   }
   else{
      if(!empty($image)){
         mysqli_query($conn, "UPDATE `products` SET name = '$name', details = '$details', price = '$price', image = '$image' WHERE id = '$update_p_id'") or die('query failed');
